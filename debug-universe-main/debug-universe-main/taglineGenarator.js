...
document
  ...
  // });
    ...
    <div class="card h-100">
      ...
      Review Scores :
      <ol class = "list-group">
      ${review_scores.scores ? Object.keys(review_scores.scores).map(key => `<li style="list-style: none" class="list-group-item list-group-item-info d-flex justify-content-between align-items-center">${key}: <span class="badge bg-primary rounded-pill">${review_scores.scores[key]}</span></li>`).join(''): "No Found Data"}
      </ol>
    </div>
      </div>
      <button class="btn btn-info btn-lg"  role="button"
